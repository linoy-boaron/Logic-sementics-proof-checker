...
class Schema:
    ...
    def _instantiate_helper(formula: Formula,
        ...
        # base case 1 formula is a invocation of a relation which is not a template or an equality:

        if (is_relation(formula.root) and formula.root not in relations_instantiation_map.keys()) or is_equality(formula.root):
            return formula.substitute(constants_and_variables_instantiation_map, set())
            #TODO can this cause problems?

        # base case 2: formula is a nullary template
        if formula.root in relations_instantiation_map.keys():
            if not formula.arguments:

                to_ret = relations_instantiation_map[formula.root]
                forbidden_vars =set(to_ret.free_variables()).union(set(to_ret.constants())).intersection(bound_variables)
                if bool(forbidden_vars):
                    raise Schema.BoundVariableError(forbidden_vars.pop(), formula.root)
                return to_ret

            # base case 3 formula is unary template
            else:
                new_term = formula.arguments[0].substitute(constants_and_variables_instantiation_map, set())
                const_var_map_copy = {key:var for key, var in constants_and_variables_instantiation_map.items()}
                const_var_map_copy['_'] = new_term
                # check if any bound variables are contained in the variable of the instantiation
                # this next line gets all the relations that will be swapped in
                instantiation_map_relations = [relations_instantiation_map[cur_relation[0]] for cur_relation in formula.relations() if cur_relation[0] in relations_instantiation_map.keys()]
                # if any of the swapped relations has a bound variable in them, raise an exception
                for insta_relation in instantiation_map_relations:
                    bounded_added = bound_variables
                    forbidden_vars = set(insta_relation.free_variables()).union(set(insta_relation.constants())).intersection(
                        bounded_added)
                    if bool(forbidden_vars):
                        raise Schema.BoundVariableError(forbidden_vars.pop(), formula.root)

                # after doing this check we're free to substitute what we'd like, replace the _ underscore with no bound
                new_formula = relations_instantiation_map[formula.root].substitute({'_': new_term}, set())
                return new_formula
        ...
        if is_unary(formula.root):
            return Formula(formula.root, Schema._instantiate_helper(formula.first, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables))

        # binary case
        if is_binary(formula.root):
            return Formula(formula.root, Schema._instantiate_helper(formula.first, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables)
                           , Schema._instantiate_helper(formula.second, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables))

        # quantifier case
        if is_quantifier(formula.root):
            new_variable = str(constants_and_variables_instantiation_map.get(formula.variable, formula.variable))
            new_bound_variables = bound_variables | {new_variable}
            new_predicate = Schema._instantiate_helper(formula.predicate, constants_and_variables_instantiation_map,
                                                      relations_instantiation_map, new_bound_variables)
            return Formula(formula.root, new_variable, new_predicate)
    ...
    def instantiate(self, instantiation_map: InstantiationMap) -> \
        ...
        relation_map = {}
        for key in instantiation_map:
            # we already asserted all variables are variables, we need to check constants and relations
            if key not in self.templates:
                return None
            if is_variable(key):
                if type(instantiation_map[key]) is str:
                    # this is to deal with a bug in the tests..
                    const_map[key] = Term(instantiation_map[key])
                if type(instantiation_map[key]) is Term:
                    const_map[key] = instantiation_map[key]
            if is_constant(key):
                const_map[key] = instantiation_map[key]
            if is_relation(key):
                relation_map[key] = instantiation_map[key]
        try:
            return Schema._instantiate_helper(self.formula, const_map, relation_map, set())
...
class Proof:
    ...
    class AssumptionLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            # Task 9.5
            # if the line is an instantiation of its' assumptions return true
            for schema in assumptions:
                if schema.instantiate(self.instantiation_map) == self.formula:
                    return True
            return False
    ...
    class MPLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            # TODO make sure the lines themselves are also valid?

            # make sure we're not using lines in an illegal order
            if self.antecedent_line_number >= line_number or self.conditional_line_number >= line_number:
                return False

            # see if antecedent is part of the conditional
            if lines[self.antecedent_line_number].formula != lines[self.conditional_line_number].formula.first:
                return False

            # if the second half of the conditional isn't the conclusion, return false
            if lines[self.conditional_line_number].formula.second != self.formula:
                return False
    ...
    class UGLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            # make sure the formula uses A and not E (and is a quantifier)
            if self.formula.root != 'A':
                return False

            # make sure antecedent comes before
            if self.predicate_line_number >= line_number:
                return False

            # make sure the antecedent is equal to the argument in the quantified line
    ...
    class TautologyLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            prop_formula, discard_map = self.formula.propositional_skeleton()
            if is_propositional_tautology(prop_formula):
                return True
            return False
...
def axiom_specialization_map_to_schema_instantiation_map(
    ...
    # translate from skeleton to the map, then save it according to the current key in uppercase
    return {k.upper(): Formula.from_propositional_skeleton(v, substitution_map) for k, v in propositional_specialization_map.items()}
...
def prove_from_skeleton_proof(formula: Formula,
    ...
    # creating lines:
    predicate_line_formulas = [Formula.from_propositional_skeleton(line.formula, substitution_map) for line in skeleton_proof.lines]
    real_predicate_lines = []
    for predicate_formula, skeleton_line in zip(predicate_line_formulas, skeleton_proof.lines):
        # if line was proven using MP
        if skeleton_line.rule == MP:
            # if the line is MP create a predicate MP line
            real_predicate_lines.append(Proof.MPLine(predicate_formula, skeleton_line.assumptions[0], skeleton_line.assumptions[1]))

        else: # line must be from an axiom
            line_to_specialization_map = PropositionalInferenceRule.formula_specialization_map(skeleton_line.rule.conclusion, skeleton_line.formula)
            instantiation_map = axiom_specialization_map_to_schema_instantiation_map(line_to_specialization_map, substitution_map)
            real_predicate_lines.append(Proof.AssumptionLine(predicate_formula, PROPOSITIONAL_AXIOM_TO_SCHEMA[skeleton_line.rule], instantiation_map))
    # create the final proof
    return Proof(PROPOSITIONAL_AXIOMATIC_SYSTEM_SCHEMAS, formula, real_predicate_lines)
...
def prove_tautology(tautology: Formula) -> Proof:
    ...
    # Task 9.12
    prop_tautology, translation_map = tautology.propositional_skeleton()
    prop_tautology_proof = prove_propositional_tautology(prop_tautology)
